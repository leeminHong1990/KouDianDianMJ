...
class PlayerProxy(object):
	...
	def concealedKong(self, tile):
		...
		# 算分
		self.owner.cal_score(self.idx, self.idx, const.OP_CONCEALED_KONG, 1, tile)
	...
	def exposedKong(self, tile):
		...
		# 算分
		self.owner.cal_score(self.idx, self.owner.last_player_idx, const.OP_EXPOSED_KONG, 1, tile)
	...
	def continueKong(self, tile):
		...
		# 算分
		self.owner.cal_score(self.idx, fromIdx, const.OP_CONTINUE_KONG, 1, tile)
		self.owner.last_player_idx = self.idx
		if self.discard_state == const.DISCARD_FORCE:
	...
	def kong_win(self, tile, score, result):
		...
		last_player = self.owner.players_list[self.owner.last_player_idx]
		for upTile in last_player.upTiles:
			t = upTile[0]
			if t == tile and len(upTile) > 3:
				upTile.remove(tile)
		# last_player.upTiles[-1].remove(tile)
		self.owner.red_score(last_player.kong_record_list[-1][1], last_player.kong_record_list[-1][2], last_player.kong_record_list[-1][0], last_player.kong_record_list[-1][3][0])
		# if self.discard_state == const.DISCARD_FORCE:
		self.owner.broadcastOperation(self.idx, const.OP_KONG_WIN, [self.owner.players_list[self.owner.current_idx].last_draw,])
		# else:
		# 	self.owner.broadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
	...
	def drawTile(self, tile, is_first = False):
		...
		self.last_draw = tile
		self.owner.canWinTiles = self.owner.getCanWinTiles(list(self.tiles), const.OP_DRAW_WIN, self.idx)
		self.tiles.append(tile)
		...
		if self.discard_state == const.DISCARD_FORCE:
			...
			if is_win and self.owner.game_mode != const.KING_GAME_MODE:
				self.owner.add_timer(1, Functor.Functor(self.draw_win, tile, score, result))
			elif is_win and self.owner.game_mode == const.KING_GAME_MODE:
				pass
			elif self.owner.can_continue_kong(self.idx, tile):
				pass
				# self.owner.add_timer(1, Functor.Functor(self.continueKong, tile))
			elif self.owner.can_concealed_kong(self.idx, tile):
				pass
				# self.owner.add_timer(1, Functor.Functor(self.concealedKong, tile))
			else:
	...
	def forceDiscard(self):
		if self.owner.can_discard(self.idx, self.last_draw):
	...
	def discardTile(self, tile = None):
		...
		if self.discard_state == const.DISCARD_FORCE:
			self.buckle += 1
			if self.buckle == 1:
				self.owner.op_record.append((const.DISCARD_FORCE, self.idx, self.idx, [tile,]))
				self.owner.tingTileList[self.idx] = len(self.discard_tiles) - 1
			self.owner.broadcastOperation(self.idx, const.OP_DISCARD, [tile, ], self.buckle)
		else:
