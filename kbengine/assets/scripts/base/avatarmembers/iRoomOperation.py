...
class iRoomOperation(object):
	...
	def createRoom(self, create_dict):
		...
		def callback(content):
			...
			try:
				...
				card_cost, diamond_cost = switch.calc_cost(game_round, {"game_mode":game_mode, "pay_mode":pay_mode})
			...
			if room.room_type == const.AGENT_ROOM:
				if room.pay_mode == const.AGENT_PAY_MODE:
					...
					if switch.DEBUG_BASE:
						...
						utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost, callback, "KouDianDian RoomID:{}".format(room.roomID))  # reason 必须为英文
	...
	def startGame(self, dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, swap_list):
		if getattr(self, 'client', None):
			self.client.startGame(dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, swap_list)

	def postOperation(self, idx, aid, tile_list, buckle = 0):
		if getattr(self, 'client', None):
			self.client.postOperation(idx, aid, tile_list, buckle)
	...
	def setPassWinState(self, state):
		"""玩家过胡状态"""
		DEBUG_MSG("call server setPassWinState state:{0}".format(state))
		self.room and self.room.setPassWinState(self, state)

	def setDiscardState(self, state, idx):
		"""玩家出牌状态"""
		DEBUG_MSG("call server setDiscardState state:{0} idx:{1}".format(state, idx))
		self.room and self.room.setDiscardState(self, state, idx)
