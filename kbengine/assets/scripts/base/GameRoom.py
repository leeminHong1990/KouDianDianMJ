...
class GameRoom(BaseEntity, GameObject, iRoomRules):
	...
	def startGame(self, beginTilesList, swap_list):
		...
		for i,p in enumerate(self.players_list):
			if p and p.mb:
				...
				p.mb.startGame(self.dealer_idx, beginTilesList[i], self.wreathsList, self.kingTiles, self.prevailing_wind, self.windsList, diceList, swap_list)
	...
	def setPassWinState(self, avt_mb, state):
		idx = -1
		DEBUG_MSG("room:{0},curround:{1} idx:{2} setPassWinState state:{3}".format(self.roomID, self.current_round, self.current_idx, state))
		for i,p in enumerate(self.players_list):
			if p is not None and p.mb == avt_mb:
				idx = i
				break
		self.pass_win_list[idx] = state

	def setDiscardState(self, avt_mb, state, idx):
		# idx = -1
		DEBUG_MSG("room:{0},curround:{1} idx:{2} setDiscardState state:{3}".format(self.roomID, self.current_round, self.current_idx, state))
		# for i,p in enumerate(self.players_list):
		# 	if p is not None and p.mb == avt_mb:
		# 		idx = i
		# 		break
		...
		if not self.can_change_discard_state(p.tiles, idx, state):
			p.mb.doOperationFailed(const.OP_ERROR_ILLEGAL)
			return
		p.setDiscardState(state)
		...
		for i in range(len(self.wait_op_info_list)):
			...
			if wait_op_dict["idx"] == idx and wait_op_dict["state"] == const.OP_STATE_WAIT:
				if (wait_op_dict["aid"] >> 3) == const.SHOW_KONG:
					...
					self.confirmOperation(p.mb, wait_op_dict["aid"], wait_op_dict["tileList"])
				else:
					self.confirmOperation(p.mb, const.OP_PASS, wait_op_dict["tileList"])
				break

		for i,p in enumerate(self.players_list):
			if p is not None:
				p.mb.postPlayerDiscardState(idx, state)
	...
	def winGame(self, idx, op, finalTile, from_idx, score, result):
		...
		if self.players_list[from_idx].state == const.DISCARD_FREE:
	...
	def broadcastOperation(self, idx, aid, tile_list = None, buckle = 0):
		"""
		将操作广播给所有人, 包括当前操作的玩家
		...
		for i, p in enumerate(self.players_list):
			if p is not None:
				p.mb.postOperation(idx, aid, tile_list, buckle)
	...
	def waitForOperation(self, idx, aid, tile, nextIdx = -1): #  aid抢杠 杠花没人可胡 nextIdx还是自己
		...
		if sum([len(x) for x in notifyOpList]) > 0:
			...
			# 摸打模式
			for i,p in enumerate(self.players_list):
				if p is not None and len(notifyOpList[i]) > 0 and p.state == const.DISCARD_FORCE:
					wait_op_dict = None
					...
					if wait_op_dict is not None:
						if self.game_mode != const.KING_GAME_MODE and (wait_op_dict["aid"] >> 3) != const.SHOW_KONG:
							self.add_timer(1, Functor(self.confirmOperation, p.mb, wait_op_dict["aid"], wait_op_dict["tileList"]))
						# self.confirmOperation(p.mb, wait_op_dict["aid"], wait_op_dict["tileList"])
					else:
						self.confirmOperation(p.mb, const.OP_PASS, notifyOpList[i][0]["tileList"])
		else:
	...
	def get_reconnect_room_dict(self, userId):
		...
		return {
			...
			'discardStateList'		: [p.state for i,p in enumerate(self.players_list) if p is not None],
