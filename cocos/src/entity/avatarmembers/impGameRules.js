...
var impGameRules = impGameOperation.extend({
    ...
    checkCanTing:function (opDict) {
	    //轮到自己，是否可以听牌
	    opDict = opDict || {};
	    if (this.curGameRoom.discardStateList[this.serverSitNum] === const_val.DISCARD_FREE && Object.keys(opDict).length < 2) {
		    cc.log("检测是否可以听牌！");
    ...
    getDrawOpDict: function (drawTile, serverSitNum) {
        ...
        if (this.isOpLimit()) {
            //胡
            if (handTiles.length % 3 === 2 && this.canWin(handTiles, drawTile, serverSitNum, const_val.OP_DRAW_WIN) && this.curGameRoom.discardStateList[serverSitNum] === const_val.DISCARD_FORCE) {
	    ...
	    if (this.curGameRoom.discardStateList[this.serverSitNum] === const_val.DISCARD_FORCE && this.curGameRoom.game_mode === const_val.KING_GAME_MODE) {
    ...
    nomalCanWin: function (handTiles, finalTile, serverSitNum, aid) {
        ...
        if (cutil.checkIs7Pairs(handTilesButKing, kingTilesNum)) {              // 7对
            return true;
        } else if (this.curGameRoom.game_mode === const_val.SPECIAL_GAME_MODE && cutil.getThirteenOrphans(handTilesButKing, kingTilesNum)) { // 十三幺
		    return true;
