...
var impGameOperation = impCommunicate.extend({
	...
	startGame : function(dealerIdx, tileList, wreathsList, kingTiles, prevailing_wind, playerWindList, diceList, swap_list){
		cc.log("startGame")
		cc.log(dealerIdx, tileList, wreathsList, kingTiles, prevailing_wind, playerWindList, diceList, swap_list);
		var self = this;
		if(!this.curGameRoom){
			return;
		}
		...
		if(aid == const_val.OP_PASS){
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
					...
					//轮到自己摸牌，是否可以随意出牌
					if (this.curGameRoom.discardStateList[this.serverSitNum] === const_val.DISCARD_FORCE) {
			...
			if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
				...
				h1global.curUIMgr.roomLayoutMgr.notifyObserver("update_canwin_tile_panel", const_val.NOT_DISPLAY_CANWIN_PANEL);
			}
			// if(this.curGameRoom.kingTiles.indexOf(tileList[0]) >= 0){
			if(buckle === 1) {
                if(this.curGameRoom.playerInfoList[serverSitNum]["sex"] == 1){
                    cc.audioEngine.playEffect(voice_root + "male/sound_man_ting.mp3");
                } else {
                    cc.audioEngine.playEffect(voice_root + "female/sound_woman_ting.mp3");
                }
			} else {
                if(this.curGameRoom.playerInfoList[serverSitNum]["sex"] == 1){
	...
	doOperation : function(aid, tileList){
		...
		if(this.curGameRoom.curPlayerSitNum === this.serverSitNum && aid === const_val.OP_PASS){
			...
			if (this.checkPassWin) {
				this.setPassWinState(const_val.PASS_WIN);
				this.checkPassWin = false;
			}
			// return;
		}
		...
		if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow() && aid !== const_val.OP_PASS) {
			h1global.curUIMgr.roomLayoutMgr.notifyObserver("lock_player_hand_tiles");
            h1global.curUIMgr.roomLayoutMgr.notifyObserver("hide_discard_tips");
		}
		// 自己的操作直接本地执行，不需要广播给自己
		if (this.curGameRoom.discardStateList[this.serverSitNum] === const_val.DISCARD_FREE) {
	...
	waitForOperation : function(aid_list, tileList){
		...
		// 摸打模式
		if ((aid_list.indexOf(const_val.OP_DRAW_WIN) >= 0 || aid_list.indexOf(const_val.OP_KONG_WIN) >= 0 || aid_list.indexOf(const_val.OP_GIVE_WIN) >= 0) && this.curGameRoom.discardStateList[this.serverSitNum] === const_val.DISCARD_FORCE && this.curGameRoom.game_mode === const_val.KING_GAME_MODE) {
	...
	setPassWinState:function (state) {
		...
		this.baseCall("setPassWinState", state);
	},

	setDiscardState : function(state, idx){
		if (!this.curGameRoom) {
			return;
		}
		idx = idx || this.serverSitNum;
		this.curGameRoom.discardStateList[this.serverSitNum] = state;
		cc.log("setDiscardState:",state,idx);
		this.baseCall("setDiscardState", state, idx);
	},
	...
	roundResult : function(roundRoomInfo){
		...
		if(h1global.curUIMgr && h1global.curUIMgr.gameRoomUIIsShow) {
			var func = function () {
