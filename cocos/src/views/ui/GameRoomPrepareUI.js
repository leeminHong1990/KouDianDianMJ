...
var GameRoomPrepareUI = UIBase.extend({
	...
	initUI:function(){
		...
		//var share_title = ' ' + switches.gameName + (player.curGameRoom.club_id > 0 ? '茶楼号【' + player.curGameRoom.club_id.toString() + '】' : '房间号【' + player.curGameRoom.roomID.toString() + '】');
        var share_title = ' 扣点点麻将 '+ (player.curGameRoom.club_id > 0 ? '茶楼号【' + player.curGameRoom.club_id.toString() + '】' : '房间号【' + player.curGameRoom.roomID.toString() + '】');
		...
		wxinvite_btn.addTouchEventListener(function(sender, eventType){
			if(eventType == ccui.Widget.TOUCH_ENDED){
			    //获取需要三缺一信息
                var str = self.get_add_share_title();
                var add_share_title = str+share_title;
			    cc.log(add_share_title);
                ...
                if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative)){
					jsb.reflection.callStaticMethod(switches.package_name + "/AppActivity","callWechatShareUrl", "(ZLjava/lang/String;Ljava/lang/String;Ljava/lang/String;)V", true, share_url, add_share_title, share_desc);
				} else if((cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){
					jsb.reflection.callStaticMethod("WechatOcBridge","callWechatShareUrlToSession:fromUrl:withTitle:andDescription:", true, share_url, add_share_title, share_desc);
				} else {
					cutil.share_func(add_share_title, share_desc);
    ...
    get_share_desc : function () {
	    ...
	    var roominfo_list = [["普通玩法","特殊牌型玩法","耗子玩法"],["风耗子", "随机耗子"],["非赏金","赏金"]];
	    var share_list = [];
